...
#if 0
// --------------------------------------------------------------------------------------
// Surface Klasse f�r das Laden und Anzeigen (copyrects)
...
public:
    ...
    bool LoadImage(const char *Filename,					// Laden des Bildes "Filename"
                   int xSize, int ySize);					// mit Gr�sse xSize, ySize
...
// --------------------------------------------------------------------------------------
// Sprite Klasse f�r das Laden und Anzeigen
// von Sprites (MIT Colorkey und anderen Effekten, hoffentlich schnell :)
// mit Hilfe von Vertices und Texturen dr�bergeklebt
// --------------------------------------------------------------------------------------
...
public:
    ...
    int16_t				itsXFrameSize;						// Framegr�sse in X-Richtung
    int16_t				itsYFrameSize;						// Framegr�sse in Y-Richtung
    ...
    //RECT				itsPreCalcedRects[256];				// vorberechnete Ausschnitte f�r die Frames
#ifdef _DEBUG
    ...
    RECT				*itsPreCalcedRects;				// vorberechnete Ausschnitte f�r die Frames
#endif
    ...
    bool  LoadImage(const std::string &filename,							// Laden des Bildes "Filename"
                    uint16_t xs, uint16_t ys, uint16_t xfs, uint16_t yfs,		// x/y Size und Framegr�sse
    ...
    void  RenderSpriteScaled(float x, float y,				// Sprite in beliebiger Gr�sse
                             int width, int height,
                             D3DCOLOR col);

    void  RenderSpriteScaled(float x, float y,				// Sprite in beliebiger Gr�sse mit Animationsphase
                             int width, int height,
                             int Anim,
                             D3DCOLOR col);
    ...
    void  RenderSpriteScaledRotated(float x, float y,		// beliebige Gr�sse und rotiert
